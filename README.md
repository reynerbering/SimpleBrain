...
| Protocol | Owns | Scope |
| --- | --- | --- |
| [workflow.md](protocols/workflow.md) | The two-phase workflow, Phase 1 and Phase 2 steps, **branch and worktree naming**, **where work lives** (`/in-progress` → `/wiki/<WEEK>/<TEAM>/`), what `done` means and the `later` flag, where each artifact ends up | travels |
...
| Artifact | Pattern |
| --- | --- |
| Design doc | `in-progress/<TEAM>/<JIRA-KEY>-<slug>.md`, then `wiki/<WEEK>/<TEAM>/` once marked done |
